-- Merges two contact records into one - used when the same person ends up
-- with two rows (e.g. Quo auto-creates a new contact from a phone number
-- that isn't on file yet as anyone's primary or secondary number). Runs as
-- a single transaction via security definer so a partial failure can't
-- leave activities/deals split across two contacts. keep_id survives;
-- merge_id's data is folded in (only filling genuinely blank fields, never
-- overwriting what's already on keep_id) and the row is deleted at the end.
...
begin
  ...
  set
    ...
    -- 'unknown' is the field's own placeholder for "not set yet", not a
    -- real value - treat it as blank so a real timeline from either side
    -- of the merge wins over an unset default.
  ...
  -- tags: copy over anything the surviving contact doesn't already have
  -- (the on-insert trigger re-derives drip enrollment from this, so no
  -- separate enrollment copy is needed for tag-driven sequences)
  ...
  -- sequence tables have (sequence/step, contact) uniqueness - drop the
  -- merge_id side wherever the keep_id side already has that same row
