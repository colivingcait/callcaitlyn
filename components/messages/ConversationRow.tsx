...
export function ConversationRow({ conversation, stages }: { conversation: Conversation; stages: PipelineStage[] }) {
  ...
  const status = typeof lastActivity.metadata?.status === "string" ? lastActivity.metadata.status.toLowerCase() : null;
  const isMissedCall = lastActivity.type === "call" && lastActivity.direction === "inbound" && status != null && MISSED_STATUSES.has(status);
  ...
  return (
    <div className="flex items-center gap-1 border-b border-neutral-100 pr-2 hover:bg-neutral-50">
      <Link href={`/messages/${contact.id}`} className="flex min-w-0 flex-1 items-center gap-3 px-4 py-3">
        <Avatar id={contact.id} firstName={contact.first_name} lastName={contact.last_name} />
        <div className="min-w-0 flex-1">
          <p className="truncate font-medium text-neutral-900">{fullName(contact)}</p>
          <div className={cn("flex items-center gap-1 text-xs", isMissedCall ? "font-medium text-red-600" : "text-neutral-500")}>
            <Icon size={12} className={cn("shrink-0", isMissedCall ? "text-red-500" : "text-neutral-400")} />
          ...
          {(stage || likelihood || contact.contact_tags.length > 0) && (
