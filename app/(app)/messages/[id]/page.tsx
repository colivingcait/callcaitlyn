...
export default async function MessageThreadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [contact, thread] = await Promise.all([getContact(id), getContactThread(id)]);

  if (!contact) notFound();

  return (
    <div className="mx-auto flex max-w-2xl flex-col">
      <div className="sticky top-0 z-10 flex items-center gap-3 border-b border-neutral-200 bg-white/95 px-3 py-2.5 backdrop-blur">
