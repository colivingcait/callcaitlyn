...
- **Bulk import** — paste rows copied straight from a spreadsheet (a header row + data rows, tab- or comma-separated). It recognizes Address/Client Name, Closing Date, Sale Price, Gross Comp, Side, Referral %, Misc, OZ, and FMLS by column name — case-insensitive, and tolerant of copying your *entire* existing commission tracker as-is, since anything it doesn't recognize (KW, KWRI, Total Fees, Net Comm, etc.) is just ignored and recalculated fresh instead. Shows a preview (with any skipped rows and why) before you confirm the import.

Either way, these count toward the KW/KWRI caps for whichever commission year their closing date falls in, exactly like any other deal, and can be edited or removed later from the pencil/✕ icons in the Commissions table itself, since they have no contact profile to manage them from. Run [`supabase/migrations/0011_standalone_deals.sql`](./supabase/migrations/0011_standalone_deals.sql) to make `contact_id` optional on deals, and [`supabase/migrations/0012_fmls_toggle.sql`](./supabase/migrations/0012_fmls_toggle.sql) to add the "On FMLS" checkbox.
