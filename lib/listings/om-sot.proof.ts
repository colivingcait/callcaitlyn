...
assert.ok(omPage.includes("which is not always what a T12 shows."));
assert.ok(omPage.includes(">T12<"));
assert.equal(omPage.toLowerCase().includes("trailing twelve months"), false, "public OM occupancy copy uses T12, not the expansion");
...
assert.ok(gate.includes("MONTHLY AVERAGE · T12"));
assert.ok(gate.includes("Figures are T12 monthly averages"));
assert.ok(gate.includes("Every figure above is a T12 monthly average."));
assert.equal(gate.toLowerCase().includes("trailing twelve months"), false, "gated OM copy uses T12, not the expansion");
