...
export type NewLeadContact = Pick<Contact, "id" | "first_name" | "last_name" | "phone" | "lead_source" | "lead_date" | "stage_id">;
...
export async function listNewLeadsQueue(): Promise<{ contacts: NewLeadContact[]; error: string | null }> {
  ...
  const { data: candidates, error: candidatesError } = await supabase
    .from("contacts")
    .select("id, first_name, last_name, phone, lead_source, lead_date, stage_id")
    .eq("archived", false)
